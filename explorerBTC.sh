...
function helpPanel(){
    ...
    echo -e "\t${Cyan}[${LYellow}-e${Cyan}]${LGray} Modo exploración${Color_Off}"
    echo -e "\t\t${LRed}unconfirmed_transactions${LGray}:\t Listar transacciones no confirmadas; resultados por default: 100${Color_Off}"
    echo -e "\t\t${LRed}inspect${LGray}:\t\t\t Inspeccionar un hash de transacción${Color_Off}"
    echo -e "\t\t${LRed}address${LGray}:\t\t\t Inspeccionar una transacción de dirección${Color_Off}"
    echo -e "\t${Cyan}[${LYellow}-n${Cyan}]${LGray} Limitar el número de resultados${Color_Off}"
    echo -e "\t${Cyan}[${LYellow}-i${Cyan}]${LGray} Proporcionar el identificador de transacción${Color_Off}"
    echo -e "\t${Cyan}[${LYellow}-a${Cyan}]${LGray} Proporcionar una dirección de transacción${Color_Off}"
    echo -e "\t${Cyan}[${LYellow}-h${Cyan}]${LGray} Mostrar este panel de ayuda${Color_Off}"
    echo -e "\n${BGray}EJEMPLOS:${Color_Off}"
    echo -e "\t${LGray}./explorerBTC ${LYellow}-e ${LRed}unconfirmed_trasactions ${LYellow}-n ${LGray}10${Color_Off}"
    echo -e "\t${LGray}./explorerBTC ${LYellow}-e ${LRed}inspect ${LYellow}-i ${LGray}2sd12d34f1s1123hjty543sdf${Color_Off}"
    echo -e "\t${LGray}./explorerBTC ${LYellow}-e ${LRed}address ${LYellow}-a ${LGray}adsd34a3a2as4da2lugdf565f${Color_Off}\n"
